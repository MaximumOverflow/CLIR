...
impl MetadataTableImpl<'_> for AssemblyTable {
...
impl ParseRow for ModuleTable<'_> {
	type Row = Module;

	fn parse_row(&self, reader: &mut ByteStream) -> Result<Self::Row, Error> {
		Ok(Module {
			generation: reader.read::<u16>()?,
			name: reader.read_index(self.str_size)?,
			module_version_id: reader.read_index(self.guid_size)?,
			enc_id: reader.read_index(self.guid_size)?,
			enc_base_id: reader.read_index(self.guid_size)?,
		})
	}
}

impl<'l> MetadataTableImpl<'l> for ModuleTable<'l> {
	fn cli_identifier() -> TableKind {
		TableKind::Module
	}
...
pub struct TypeRefIterator<'l> {
	reader: ByteStream<'l>,
	table: TypeRefTable<'l>,
}

impl <'l> MetadataTable<'l> for TypeRefTable<'l> {
	type Iter = TypeRefIterator<'l>;
...
impl ParseRow for TypeRefTable<'_> {
	type Row = TypeRef;

	fn parse_row(&self, reader: &mut ByteStream) -> Result<Self::Row, Error> {
		Ok(TypeRef {
			resolution_scope: reader.read_index(self.res_size)?,
			type_name: reader.read_index(self.str_size)?,
			type_namespace: reader.read_index(self.str_size)?,
		})
	}
}

impl<'l> MetadataTableImpl<'l> for TypeRefTable<'l> {
	fn cli_identifier() -> TableKind {
		TableKind::TypeRef
	}
...
pub struct TypeDefIterator<'l> {
	reader: ByteStream<'l>,
	table: TypeDefTable<'l>
}

impl <'l> MetadataTable<'l> for TypeDefTable<'l> {
	type Iter = TypeDefIterator<'l>;
...
impl ParseRow for TypeDefTable<'_> {
	type Row = TypeDef;

	fn parse_row(&self, reader: &mut ByteStream) -> Result<Self::Row, Error> {
		Ok(TypeDef {
			flags: reader.read::<TypeAttributes>()?,
			name: reader.read_index(self.str_size)?,
			namespace: reader.read_index(self.str_size)?,
			extends: reader.read_index(self.ext_size)?,
			fields: reader.read_index(self.fld_size)?,
			methods: reader.read_index(self.mtd_size)?,
		})
	}
}

impl<'l> MetadataTableImpl<'l> for TypeDefTable<'l> {
	fn cli_identifier() -> TableKind {
		TableKind::TypeDef
	}
...
pub struct MethodDefIterator<'l> {
	reader: ByteStream<'l>,
	table: MethodDefTable<'l>,
}

impl <'l> MetadataTable<'l> for MethodDefTable<'l> {
	type Iter = MethodDefIterator<'l>;
...
impl ParseRow for MethodDefTable<'_> {
	type Row = MethodDef;

	fn parse_row(&self, reader: &mut ByteStream) -> Result<Self::Row, Error> {
		Ok(MethodDef {
			rva: reader.read::<u32>()?,
			impl_flags: reader.read::<MethodImplFlags>()?,
			flags: reader.read::<MethodFlags>()?,
			name: reader.read_index(self.str_size)?,
			signature: reader.read_index(self.blob_size)?,
			params: reader.read_index(self.param_size)?,
		})
	}
}

impl<'l> MetadataTableImpl<'l> for MethodDefTable<'l> {
	fn cli_identifier() -> TableKind {
		TableKind::MethodDef
	}
...
impl ParseRow for FieldTable<'_> {
	type Row = Field;

	fn parse_row(&self, reader: &mut ByteStream) -> Result<Self::Row, Error> {
		Ok(Field {
			flags: reader.read::<FieldFlags>()?,
			name: reader.read_index(self.str_size)?,
			signature: reader.read_index(self.blob_size)?,
		})
	}
...
impl<'l> MetadataTableImpl<'l> for FieldTable<'l> {
	fn cli_identifier() -> TableKind {
		TableKind::Field
	}
...
macro_rules! impl_metadata_iter {
    ($([$Iter: ty, $Item: ty]),+ $(,)?) => {
		$(
...
impl_metadata_iter!(
	[FieldIterator<'_>, Field],
	[ModuleIterator<'_>, Module],
	[TypeRefIterator<'_>, TypeRef],
	[TypeDefIterator<'_>, TypeDef],
	[MethodDefIterator<'_>, MethodDef],
);
