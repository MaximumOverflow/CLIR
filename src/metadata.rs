...
use crate::tables::{
	AssemblyTable, GenericMetadataTable, Module, ModuleTable, TableKind, TableKindIter, TypeDef, TypeDefTable, TypeRef,
	TypeRefTable,
};
...
impl<'l> MetadataHeader<'l> {
	...
	pub(crate) fn get_heaps<'a>(&self, assembly_bytes: &'a [u8]) -> HashMap<TypeId, Box<dyn MetadataHeap<'a> + 'a>> {
...
impl<'l> Iterator for StreamHeaderIterator<'l> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		let offset = *self.reader.read::<u32>().ok()?;
		let size = *self.reader.read::<u32>().ok()?;
		let name = self.reader.read_until(0);
		self.reader.skip((4usize.wrapping_sub(name.len())) % 4).ok()?;
		...
		loop {
			...
			let table = match kind {
				TableKind::Module => {
					...
					ModuleTable::new(self.reader.read_slice::<u8>(rows * row_size).ok()?, guid_idx_size, str_idx_size)
						.into()
				}
				...
				TableKind::TypeRef => {
					...
					TypeRefTable::new(self.reader.read_slice::<u8>(rows * row_size).ok()?, res_idx_size, str_idx_size)
						.into()
				}

				TableKind::TypeDef => {
					let str_idx_size = self.heap.heap_idx_size::<StringHeap>();
					let ext_idx_size = get_coded_index_size(CodedIndexKind::TypeDefOrRef, &self.heap);
					let fld_idx_size = self.heap.table_idx_size(TableKind::Field);
					let mtd_idx_size = self.heap.table_idx_size(TableKind::Method);
					let row_size = TypeDef::row_size(str_idx_size, ext_idx_size, fld_idx_size, mtd_idx_size);
