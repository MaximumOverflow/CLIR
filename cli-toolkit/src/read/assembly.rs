use crate::raw::{AssemblyRefTable, AssemblyTable, BlobHeap, CodedIndexKind, MetadataHeap, MetadataTable, MetadataTableImpl, StringHeap, TableHeap, TableIndex, TypeDefTable, TypeRefTable};
...
impl Assembly<'_> {
	...
	pub(crate) fn read<'l>(assembly: &'l mut Assembly<'l>, path: PathBuf) -> Result<AssemblyReader<'l>, Error> {
...
impl <'l> AssemblyReader<'l> {
	pub(crate) fn get_ident(&self) -> Result<String, Error> {
		let raw = crate::raw::Assembly::try_from(self.bytes.as_slice())?;
		let tables = raw.get_heap::<TableHeap>()?.ok_or(Error::MissingMetadataHeap(TableHeap::cli_identifier()))?;
		let strings = raw.get_heap::<StringHeap>()?.ok_or(Error::MissingMetadataHeap(StringHeap::cli_identifier()))?;

		let def = tables.get_table::<AssemblyTable>()?
	...
	pub(crate) fn read(mut self) -> Result<(), Error> {
		...
		let raw = crate::raw::Assembly::try_from(bytes.as_slice())?;

		let blobs = raw.get_heap::<BlobHeap>()?.ok_or(Error::MissingMetadataHeap(BlobHeap::cli_identifier()))?;
		let tables = raw.get_heap::<TableHeap>()?.ok_or(Error::MissingMetadataHeap(TableHeap::cli_identifier()))?;
		let strings = raw.get_heap::<StringHeap>()?.ok_or(Error::MissingMetadataHeap(StringHeap::cli_identifier()))?;
	...
	fn read_assembly_type_refs(&mut self, strings: StringHeap, tables: TableHeap) -> Result<(), Error> {
		...
		for ty in table.iter() {
			...
			let token = ty.resolution_scope().decode(CodedIndexKind::ResolutionScope)
